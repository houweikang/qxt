...
from excel_style import add_title_and_subtitle,hour_style
...
def fun_hour_report(date_hour, dq, df_target=target()):
    ...
    result.to_excel(file_fullname, sheet_name=sheet_name, startrow=2)

    #统计时间段
    dt = dt.replace('-','/')
    dts = '{} 0:00-{}:00'.format(dt,h)
    return file_fullname,sheet_name,dts
...
def hour_report():
    ...
    for dq in dq_list:
        wb_fullname,sht_name,dts = fun_hour_report(date_hour, dq)
        add_title_and_subtitle(wb_fullname,sht_name,sht_name,dts)
        hour_style(wb_fullname,sht_name)
    only_ok()
